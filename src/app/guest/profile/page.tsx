...
function passportConfig(status: PassportStatus) {
  const map: Record<
    PassportStatus,
    {
      label: string;
  ...
  > = {
    verified: {
      label: "Verified",
      description: "Your identity has been verified. You're all set for booking.",
    ...
    none: {
      label: "Not Submitted",
      description:
        "Passport verification is required before booking. Submit your documents to get started.",
...
export default async function GuestProfilePage() {
  ...
  const passport = passportConfig(passportStatus);
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="hidden sm:flex flex-col items-end gap-1.5">
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#C9A84C]/10 border border-[#C9A84C]/20 rounded-full text-[11px] font-black uppercase tracking-wider text-[#C9A84C]">
                ...
                {role === "host" ? "Host" : role === "admin" ? "Admin" : "Guest"}
              </span>
            </div>
          </div>
        </div>
